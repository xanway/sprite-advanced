...
1. 在flex布局中使用flex-wrap样式时，注意，该样式一般用于水平布局，如果布局容器是垂直布局，默认每个控件必须换行，不能用这个样式，否者可能照成页面无法滚动。  
...
9.  opacity设置的是整个控件的透明，如果控件是容器，那么容器内部所有的控件都会跟着一起变透明。如果想实现背景色透明，可以使用background-color:rgba(rgba(255, 0, 0, 0.5))来设置。  

10.  如果想设置文字为省略样式，首先必须设置文本单行显示，如text样式singleline：true，并且text宽度应该小于等于父容器的宽度。  

11. 使用display样式显隐控件后，需要调用document.refresh()刷新布局，如果容器区域大小确定，可以可以对容器内部刷新 xxxdom.refresh()；  
...
24. 每个控件都有自己的refresh()刷新方法，建议控件内部刷新，都使用局部刷新，但是要保证控件自身区域大小固定，该刷新只会影响控件内部，外部布局不会变动。注意，局部刷新只刷控件内部，不刷新控件自身。  

25. 尽可能少用全局刷新document.refresh(), 改刷新在android上会有效率问题。除非发生页面整体布局变动。  

26. setClassStyle该法设置class后，以前的通过class设置的样式会全部清除，不过style设置的不会清除。  

27. 由于style的优先级要高于class，如果class设置的样式里面，包含之前style设置的样式，还是以style中的样式为优先。  

28. 在通过getStyle获取color的时候，（如：#ff0000），在android上由于css引擎的差异性，通过getStyle("color")获取的颜色值是rgb(255,0,0)这种形式，通过js设置的颜色值则没有影响。  

29. appendChild()用于向容器中添加节点对象，执行该方法，需要刷新容器布局方可生效。  

30. 通过appendChild()向容器添加节点的时候，如果该容器正在监听touch系列事件，会造成事件中断，比如正在监听一个容器的touchMove拖动事件，在拖动的过程中，向该容器内部添加节点，会照成touchMove事件中断。  
...
34.  image控件必须指定高和宽后才能显示，如果想动态算出image的高宽进行布局，可以监听loaded事件，该事件会返回图片的高和宽，然后在动态修改image的style，如果页面内部有多张图片，可以利用事件冒泡机制，可在外层容器上监听loaded事件。  

35.  使用scroll时需要设置其区域大小，一般情形下要设置高宽，也可以用flex来分割区域，如果不设置区域大小，其内容无非显示。  

36. refresh虽然为独立控件，但是在list控件内部，属于list内部子控件，对应子控件，是不能单独封装成模板组件的，如果要封装，也只能封装refresh内部的布局控件。  

37. 如果scroll和list内部数据动态加载，如果想在加载完成后马上自动滚动到某一区域，这需要通过定时器延迟下，在执行滚动操作，因为刷新布局会消耗时间，如果刷新后里面执行滚动在android上可能没有效果。  

38. refresh的reset()复位方法一定要延迟执行才有效果，因为里面有一个刷新动画，一般情况下都是发起网络请求后才执行reset()方法，自然不用关心，如果做演示数据，需要人为的加一下定时器。  

39. 使用list时需要设置其区域大小，一般情形下要设置高宽，也可以用flex来分割区域，如果不设置区域大小，其内容无非显示。  

40. 在list的cell内部不能做有关dom的容器操作，比如动态的添加一个dom或者隐藏一个dom（不过可以使用visibility来做控件的隐藏），总之在cell内部不允许有修改区域大小的操作。    

41. ListAdapter适配器是专门用于list控件的数据绑定的，其内部会做数据缓存处理可以支持超大数据量的列表数据，建议大数量列表用list来完成。  

42. 在使用 ListAdapter的时候， 如果使用notifyItemRangexxxx系列的局部刷新（删除，修改，添加），要保证数据源对应进行删除，修改，添加操作，否者会出错。  

43. 使用grid时需要设置其宽度，也可以用flex来分割区域，如果不设置宽度，其内容无非显示。  

44. iconfont文字图组件用于文字图展示，支持通过css样式font-family设置需要加载字体库后使用，格式为\uxxxx，如：\ue674 。如果在js里面写，需要写\\ue674。  

45. 在使用webview的时候，需要注意，uixml相对html来说是窗口的概念，html页面内部跳转都将在窗口内部跳转，如果窗口关闭则整个html页面全部关闭。  

46. handsign手写签名组件不要在可滑动的容器中使用该控件，比如scroll，slider等。  

47. 所有的dom相关操作必须放在window的loaded事件之后才能执行。  
...
6. 模板外部的样式都会自动压入模板内部的第一个布局标签上面，比如button组件的样式会自动作用到button.component里面的最外层的box上，属性也是一样但除了id,style,class这三个固定属性。
...
14. 组件虽然可以帮助我们解决代码重复问题，但是不建议在一个页面里面大量使用，尽量保证一个页面里面不要超过20个封装的组件，否者在android上页面打开会有效率问题。比如在页面里面放30个button组件（在list控件里面无所谓），页面打开可能就会比较慢。简单的场景下需要重复使用某一个布局时，尽量用box进行布局，如果实在不行，可以考虑动态布局，需要用的时候在加载，不要一次性都写在页面里面。
...
4. sprite中有很多方法都是带回调方法的，回调都是异步执行的，所以在用回调方法的时候，后续方法需要放在回调里面执行，否者回调还没有结束，可能有些值是取不到的。
